...
int applypatch_check(const char* filename,
    ...
    if (LoadFileContents(filename, &file, RETOUCH_DO_MASK) != 0 ||
        (num_patches > 0 &&
         FindMatchingPatch(file.sha1, (const char **)patch_sha1_str, num_patches) < 0)) {
        ...
        if (FindMatchingPatch(file.sha1, (const char **)patch_sha1_str, num_patches) < 0) {
...
int applypatch(const char* source_filename,
    ...
    if (source_file.data != NULL) {
        int to_use = FindMatchingPatch(source_file.sha1,
                                       (const char **)patch_sha1_str, num_patches);
        if (to_use >= 0) {
...
static int GenerateTarget(FileContents* source_file,
    ...
    if (output < 0) {
        ...
        /* commnet out for test patched file integretion
