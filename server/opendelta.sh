...
INPUT_BUFF=$(nextPowerOf2 $(getFileSize work/current.zip));
$BIN_XDELTA -B ${INPUT_BUFF} -9evfS none -s work/last.zip work/current.zip out/$FILE_LAST_BASE.update
INPUT_BUFF=$(nextPowerOf2 $(getFileSize work/current_signed.zip));
$BIN_XDELTA -B ${INPUT_BUFF} -9evfS none -s work/current.zip work/current_signed.zip out/$FILE_LAST_BASE.sign
